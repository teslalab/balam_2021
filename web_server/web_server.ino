      ...
      {                         // if there's bytes to read from the client,
        ...
        { // if the byte is a newline character
          ...
          if (currentLine.length() == 0)
          {
            ...
            if (state == 1)
            {
              client.println("<p>Neopixeles Encendidos</p>");
              client.println("<p><a href=\"/neopixel/off\"><button class=\"button button2\">OFF</button></a></p>");
            }
            else
            {
              client.println("<p>Neopixeles Apagados</p>");
              client.println("<p><a href=\"/neopixel/on\"><button class=\"button\">ON</button></a></p>");
            }
...
void neopixelOn()
{
  neopixelLEDs1.clear(); // Colocamos todos los pixels en 'off'
  neopixelLEDs2.clear(); // Colocamos todos los pixels en 'off'
  ...
  neopixelLEDs1.show(); // Mandamos el update del color al hardware.
  neopixelLEDs2.show(); // Mandamos el update del color al hardware.
}
void neopixelOff()
{
  neopixelLEDs1.clear(); // Colocamos todos los pixels en 'off'
  ...
  for (int i = 0; i < PIXEL_COUNT; i++)
  {
    neopixelLEDs1.setPixelColor(i, neopixelLEDs1.Color(0, 0, 0));
    neopixelLEDs2.setPixelColor(i, neopixelLEDs2.Color(0, 0, 0));
  }
  neopixelLEDs1.show(); // Mandamos el update del color al hardware.
  neopixelLEDs2.show(); // Mandamos el update del color al hardware.
}